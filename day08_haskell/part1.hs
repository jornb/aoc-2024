...
getAntinodesForPair :: Coordinates -> Coordinates -> Set.Set Coordinates
getAntinodesForPair (x1, y1) (x2, y2) = 
    let dx = x2 - x1
        dy = y2 - y1
    in Set.fromList [(x1 - dx, y1 - dy), (x2 + dx, y2 + dy)]
...
solveAntonodes coordinates width height = foldl processPair (Set.empty, width, height) (makePairs coordinates)
    where
        processPair (antinodes, width, height) (p1, p2) =
            let newAntinodes = Set.filter (\coord -> isInRange coord width height) (getAntinodesForPair p1 p2)
