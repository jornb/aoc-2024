...
func solve(target int, numbers []int, accumulator int, index int) bool {
    ...
    n := numbers[index];
    if (solve(target, numbers, accumulator + n, index + 1) || solve(target, numbers, accumulator * n, index + 1)) {
        return true;
    }

    x, _ := strconv.Atoi(fmt.Sprintf("%d%d", accumulator, n))
    return solve(target, numbers, x, index + 1)
}
